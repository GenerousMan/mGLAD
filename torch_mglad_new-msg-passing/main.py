...
class GLADLinkPredict(nn.Module):
    ...
    def cal_score(self, ndata_, triplets):
        ...
        score = F.softmax(score, dim=1)  # E * L, p(y_ji)  [4212,2]
        return score
    ...
    def get_loss(self, g, triplets):
        ...
        predict_edges = np.argmax(score.detach().numpy(), axis=1)  #[4212,]
        predict_edge_acc = np.equal(predict_edges, rel.squeeze()).sum() / predict_edges.shape[0]

...
class mGLAD(Model):
    ...
    def __init__(self, placeholders, worker_num, task_num, input_dim, edge_type, ability_num,edge_latent_num, **kwargs):
    ...
    def _loss(self):
        def Cal_ProbLoss(loss, P, edges):
            ...
            def body_worker(i, loss_now):
                ...
                if(self.graph[edges[i][0]][edges[i][1]]!=-1):
                    loss_now = tf.add(loss_now, P[edges[i][0]][edges[i][1]][edges[i][2]])
    ...
    def _accuracy(self):
        ...
        with tf.name_scope('accuracy'):
            self.accuracy = tf.reduce_mean(tf.cast(
                tf.equal(tf.cast(tf.argmax(self.outputs, 2), float), tf.cast(self.placeholders['graph'], float)),
    ...
    def _build(self):
        ...
        self.layers.append(mpnn(input_dim=self.input_dim,
                                ...
                                tsk_num_for_wkr=self.tsk_num_for_wkr,
                                edge_latent_num=self.edge_latent_num,

...
class mpnn(Layer):
    ...
    def __init__(self, input_dim, tsk_num_for_wkr,task_num, worker_num, edge_type, ability_num,edge_latent_num, output_dim, placeholders, update_step,
                 **kwargs):
        ...
        self.edge_latent_num=edge_latent_num
        self.tsk_num_for_wkr=tsk_num_for_wkr
        ...
        with tf.variable_scope('mpnn_vars'):
            ...
            self.Vars["Awij"] = tf.Variable(
                initial_value=tf.truncated_normal(shape=[edge_type, ability_num, self.edge_latent_num], mean=0, stddev=0.01),
                name="Awij")
            # 在这里的形状是（ability类别*edge类别），即10*2
            self.Vars["Awij_latent"] = tf.Variable(
                initial_value=tf.truncated_normal(shape=[edge_type, self.edge_latent_num, edge_type], mean=0, stddev=0.01),
                name="Awij_latent")
    ...
    def _call(self, inputs):
        # inputs现在是一个记录了edge_list的列表
        # inputs'shape=[edges' num, 3]
        # inputs 的结构：wkr's number, tsk's number, class


        first_a = tf.random_normal(shape=[self.worker_num, self.ability_num], stddev=1, seed=1,
                                   name='first_a')
        first_t = tf.random_normal(shape=[self.task_num, self.edge_type], stddev=1, seed=1,
                                   name='first_t')
        ...
        def body(i, update_a, update_t):
            def cond_a(j, update_a, update_t):
                # 判断a的更新进行完毕与否
                return j < inputs.shape[0]

            def cond_t(k, update_a, update_t):
                # 判断t的更新进行完毕与否

                return k < inputs.shape[0]

            def body_a(j, update_a, update_t):
            # 用于循环迭代每一轮a的更新
                now_task_num=inputs[j][1]
                now_wker_num=inputs[j][0]
                now_edge_num=inputs[j][2]

                now_t = update_t[now_task_num]  # 根据这条边是啥取用指定tau里的值，维度 1*2
                print(update_t.shape)

                A2_label = self.Vars["Awij2"][now_edge_num]
                # 根据不同label 选用不同的A2
                print(A2_label.shape)

                update_a = tf.reshape(
                    tf.concat(
                        [
                            update_a[:now_wker_num],
                            tf.add(update_a[now_wker_num],
                                   tf.matmul(tf.reshape(now_t, [1, self.edge_type]), A2_label),
                                   name='each_tau_x_its_A2'),
                            update_a[now_wker_num + 1:]
                        ], axis=0, name='now_update_a'),
                    (self.worker_num, self.ability_num)
                )
                print(update_a.shape)
                # update_a_final=tf.concat(update_a_final,update_aj,axis=0)
                # 将update_a的第j个worker的得分进行一个累加计算
                #update_a=update_a*(tf.cast(self.task_num,tf.float32)/tf.cast(self.tsk_num_for_wkr,tf.float32))

                return j + 1, update_a, update_t

            def body_t(k, update_a, update_t):
                now_task_num = inputs[k][1]
                now_wker_num = inputs[k][0]
                now_edge_num = inputs[k][2]
                # 用于循环迭代每一轮t的更新,迭代task个数次，是第二个维度
                A_label = self.Vars["Awij"][now_edge_num]
                A_latent = self.Vars["Awij_latent"][now_edge_num]
                # 根据不同label取用不同矩阵，存在本次A_label中

                update_t = tf.reshape(
                    tf.concat(
                        [
                            update_t[:now_task_num],
                            tf.add(update_t[now_task_num],  # 求和的过程，从kk=0到kk=最后一个worker的序号，都给加上
                                   tf.matmul(tf.matmul(tf.reshape(update_a[now_wker_num], shape=[-1, self.ability_num]),
                                             A_label),A_latent)
                                   , name='each_a_x_its_A'),
                            update_t[now_task_num + 1:]
                        ],
                        axis=0), (self.task_num, self.edge_type), name='now_update_t')

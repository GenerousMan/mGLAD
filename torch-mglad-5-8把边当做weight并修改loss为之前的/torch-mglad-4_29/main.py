...
class GLADLinkPredict(nn.Module):
    def __init__(self, num_nodes, num_wkr, num_tsk, num_rels, wkr_dim, tsk_dim, mv_results, num_layers, edges,
                 dropout=0,
        ...
        # TODO:添加了initial_features，把一开始的tau 和 ability都视为参数，维度设为一致的1*L
        self.initial_features = nn.Parameter(torch.Tensor(num_nodes, 1, num_rels))  # nodes features 1*L
        nn.init.xavier_uniform_(self.initial_features, gain=nn.init.calculate_gain('relu'))
        ...
        self.mglad = MGLAD(num_nodes=num_nodes, num_wkr=num_wkr, num_tsk=num_tsk, wkr_dim=wkr_dim, tsk_dim=tsk_dim,
                           num_rels=num_rels, num_layers=num_layers)

    # TODO: 用initial_features来给tau和ability赋值
    def initial_node_features(self, g):
        g.nodes[range(self.num_wkr)].data['ability'] = self.initial_features[range(self.num_wkr)]
        g.nodes[range(self.num_wkr, self.num_nodes)].data['labels'] = self.initial_features[
            range(self.num_wkr, self.num_nodes)]
        return g
    ...
    def cal_score(self, ndata_, triplets):
        ...
        sig_aw_b = F.sigmoid(
            torch.matmul(wkr_feature, self.weight) + self.bias)

        score = sig_aw_b.mul(tsk_feature) + ((1 - sig_aw_b)/(self.num_rels-1)).mul(1 - tsk_feature)
    ...
    def forward(self, g):
        # TODO: 初始化tau和ability
        g = self.initial_node_features(g)
    ...
    def get_loss(self, g, triplets):
        ...
        loss_sum = predict_loss + 0.005 * reg_loss  # + 0.01 * kl_loss
...
def main(args):
    ...
    model = GLADLinkPredict(num_nodes=num_nodes,
                            ...
                            num_tsk=num_tsk,
                            num_rels=num_rels,
                            wkr_dim=num_rels,  # 把worker ability 的维度也设置成L维，和tau一致
                            tsk_dim=num_rels,
    ...
    while True:
        ...
        g.ndata['ability'] = torch.zeros(num_nodes, 1, num_rels)  # TODO: 把 args.wkr_dim改成num_rels
        g.ndata['deg'] = g.out_degrees(g.nodes()).float().view(-1, 1, 1)

        # g.nodes[range(num_wkr)].data['ability'] = torch.ones(num_wkr, 1, args.wkr_dim)
        # g.nodes[range(num_wkr, num_nodes)].data['labels'] = torch.ones(num_nodes - num_wkr, 1, num_rels)
...
if __name__ == '__main__':
    ...
    parser.add_argument("--gpu", type=int, default=-1,
                        help="gpu")
    parser.add_argument("--lr", type=float, default=1e-2,  # lr
                        help="learning rate")
